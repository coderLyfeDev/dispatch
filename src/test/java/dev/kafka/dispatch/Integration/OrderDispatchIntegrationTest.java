...
public class OrderDispatchIntegrationTest {
    ...
    public static class KafkaTestListener {
        ...
        AtomicInteger dispatchedCompletedCounter = new AtomicInteger(0);

        @KafkaHandler
        void receiveDispatchPreparing(@Header(KafkaHeaders.RECEIVED_KEY) String key, @Payload DispatchPrepared payload) {
        ...
        @KafkaHandler
        void receiveDispatchedCompleted(@Header(KafkaHeaders.RECEIVED_KEY) String key, @Payload DispatchCompleted payload) {
            log.debug("Received DispatchCompleted key: " + key + " payload: " + payload);
            assertThat(key, notNullValue());
            assertThat(payload, notNullValue());
            dispatchedCompletedCounter.incrementAndGet();
        }
    }
    ...
    public void setUp() {
        ...
        testListener.dispatchedCompletedCounter.set(0);
    ...
    public void testOrderDispatchFlow() throws Exception {
        ...
        await().atMost(1, TimeUnit.SECONDS).pollDelay(100, TimeUnit.MILLISECONDS)
                .until(testListener.dispatchedCompletedCounter::get, equalTo(1));
    }
